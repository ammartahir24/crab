...
'''
A class for each flow defined in config file, contains all attributes of flow needed by CRAB
'''
class Flow:
	...
	def setup(self):
		os.system("sudo tc class add dev ifb0 parent 1:1 classid 1:%d htb rate %fmbit ceil %fmbit" %(self.id, self.true_share, self.true_share))
	...
	def filters_for_ips(self):
		for ip in self.ip_addrs:
			...
			if ":" in ip:
				port = ip.split(":")[1]
				print("Port %s added with handle %d" %(port, ip_props.ip_handle))
				os.system("sudo tc filter add dev ifb0 parent 1: handle 800::%d protocol ip prio 9 u32 match ip sport %s 0xffff flowid 1:%d" %(ip_props.ip_handle, port, self.id))
			else:
				print("IP %s added with handle %d" %(ip, ip_props.ip_handle))
				os.system("sudo tc filter add dev ifb0 parent 1: handle 800::%d protocol ip prio 10 u32 match ip src %s flowid 1:%d" %(ip_props.ip_handle, ip, self.id))
	...
	def set_bandwidth(self, bw):
		...
		os.system("sudo tc class change dev ifb0 parent 1:1 classid 1:%d htb rate %fmbit ceil %fmbit" %(self.id, self.assigned_bw, self.assigned_bw))
	...
	def remove_ip_from_flow(self, ip):
		...
		if ip in self.ip_addrs:
			...
			os.system("sudo tc filter del dev ifb0 parent 1: handle 800::%d prio 10 protocol ip u32" %(self.ip_props_list[ip].ip_handle))
	...
	def add_port_to_flow(self, port):
		...
		if port not in self.ip_addrs:
			...
			os.system("sudo tc filter add dev ifb0 parent 1: handle 801::%d protocol ip prio 9 u32 match ip dport %d 0xffff flowid 1:%d" %(ip_props.ip_handle, port, self.id))
	...
	def remove_port_from_flow(self, port):
		...
		if port in self.ip_addrs:
			...
			os.system("sudo tc filter del dev ifb0 parent 1: handle 801::%d prio 9 protocol ip u32" %(self.ip_props_list[port].ip_handle))
